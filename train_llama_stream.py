"""
Train a LLaMA model with fixed learning rate for Optuna/stage1 experiments
using streaming C4 data instead of an offline token cache.
"""
...
if master_process:
    print(
        "using streaming C4 dataloaders "
        f"(shuffle_buffer_size={shuffle_buffer_size}, num_workers={dataloader_num_workers})"
    )
...
def write_experiment_summary(termination_reason, elapsed_hours):
    ...
    summary = {
        ...
        "trial_id": trial_id,
        "train_script": "train_llama_stream.py",
        "out_dir": out_dir,
...
while True:
    lr_value = get_lr(iter_num) if decay_lr else learning_rate
    for param_group, base_lr in zip(optimizer.param_groups, base_group_lrs):
        param_group["lr"] = base_lr * (lr_value / learning_rate) if learning_rate != 0 else base_lr
