"""Fine-tune GPT-2 on AG_NEWS-like classification data but only use inputs (no labels) and optimize loss."""
import argparse
import math
import os
...
def parse_args():
    ...
    p.add_argument('--out_dir', type=str, default='/scratch.global/chen8596/out/gpt2-agnews')
...
def prepare_agnews(tokenizer, block_size, split='train'):
    ds = load_dataset('ag_news', split=split)
    # Use only text field; omit label
...
def main():
    ...
    train_ids = prepare_agnews(tokenizer, args.block_size, split='train')
    val_ids = prepare_agnews(tokenizer, args.block_size, split='test')
