"""Fine-tune GPT-2 on Wikitext (language modeling) using inputs and optimize loss."""
import argparse
import math
import os
...
def parse_args():
    ...
    p.add_argument('--out_dir', type=str, default='/scratch.global/chen8596/out/gpt2-wikitext')
...
def prepare_wikitext(tokenizer, block_size, split='train'):
    ds = load_dataset('wikitext', 'wikitext-2-raw-v1', split=split)
    texts = [t for t in ds['text'] if t is not None and len(t.strip()) > 0]
...
def main():
    ...
    train_ids = prepare_wikitext(tokenizer, args.block_size, split='train')
    val_ids = prepare_wikitext(tokenizer, args.block_size, split='validation')
