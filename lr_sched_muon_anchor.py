...
class LineSearchScheduler():
    ...
    def _group_indices_for_muon_search(self):
        return self.controlled_group_indices

    def _group_indices_for_fixed_adam_anchor(self):
        return [
    ...
    @torch.no_grad()
    def apply_fixed_adam_anchor(self):
        if self.optimizer_type != "Muon":
            return

        for group_idx in self._group_indices_for_fixed_adam_anchor():
            group = self.optimizer.param_groups[group_idx]
    ...
    @torch.no_grad()
    def restore_fixed_adam_anchor(self):
        if self.optimizer_type != "Muon":
            return
    ...
    def step(self, closure, condition="armijo", c1=0.6, factor=0.5, amax=1.0, amin=1e-6, step=0, interval=100, warmup_length=100, log_dir="./", start_lr=1):
        ...
        if self.optimizer_type == "Muon":
            _ = closure(require_grad=True)
            self.rule = self.get_potential_update_direction()
            self.apply_fixed_adam_anchor()
            anchor_applied = True
            self.optimizer.zero_grad(set_to_none=True)
        ...
        if anchor_applied:
            self.restore_fixed_adam_anchor()
        self.optimizer.zero_grad(set_to_none=True)
