#!/bin/bash
#SBATCH --job-name=lsmu_gpt124m_smk
...
#SBATCH --gres=gpu:4
#SBATCH -p gpuA100x4
#SBATCH --output=exp_log/gpt124m_linesearch_muon_smoke_%A_%a.out
#SBATCH --error=exp_log/gpt124m_linesearch_muon_smoke_%A_%a.err
...
RUN_ROOT="/work/nvme/bgop/cchen47/experiment_runs/gpt124m_line_search_muon_stage2_smoke"
...
python run_linesearch_stage2.py \
  ...
  --nproc-per-node 4 \
  --experiment-name "gpt124m_line_search_muon_smoke" \
