"""Fine-tune GPT-2 on XSum-like summarization data but only use inputs (no summaries) and optimize loss."""
import argparse
import math
import os
...
def parse_args():
    ...
    p.add_argument('--out_dir', type=str, default='/scratch.global/chen8596/out/gpt2-xsum')
...
def prepare_xsum(tokenizer, block_size, split='train'):
    ds = load_dataset('xsum', split=split)

    # Use only document as input; omit summary (user requested no answers)
    texts = [f"Article: {ex['document'].strip()}\nTL;DR:" for ex in ds]
...
def main():
    ...
    train_ids = prepare_xsum(tokenizer, args.block_size, split='train')
    val_ids = prepare_xsum(tokenizer, args.block_size, split='validation')
